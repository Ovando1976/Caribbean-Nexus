...
export const usersService = {
  ...
  async createProfile(userId: string, data: Partial<UserProfile>): Promise<void> {
    const profile: UserProfile = {
      ...
      bio: data.bio || '',
      avatarUrl: data.avatarUrl || `https://picsum.photos/seed/${userId}/200/200`,
      followersCount: 0,
      followingCount: 0,
      postsCount: 0,
      createdAt: Date.now(),
...
export const postsService = {
  async createPost(userId: string, data: { caption: string, mediaType: PostType, mediaUrl?: string }): Promise<string> {
  ...
  async addComment(postId: string, userId: string, text: string): Promise<void> {
    await runTransaction(db, async (transaction) => {
      ...
      transaction.update(doc(db, 'posts', postId), { commentsCount: increment(1) });
    });
  }
};
...
export const messagesService = {
  async getOrCreateConversation(currentUserId: string, targetUserId: string): Promise<string> {
    ...
    const snapshot = await getDocs(q);
    const existing = snapshot.docs.find(doc => doc.data().memberIds.includes(targetUserId));
